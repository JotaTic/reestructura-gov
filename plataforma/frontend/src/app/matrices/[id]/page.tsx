  ...
  const handleImportXlsx = async () => {
    ...
    try {
      ...
      const result = await api.postForm<Record<string, unknown>>(
        `/matrices/${matrixId}/importar-xlsx/`,
        formData
      );
      ...
      {showImport && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
            <h2 className="mb-4 text-base font-semibold text-slate-800">Importar Matriz desde Excel</h2>
            <p className="mb-3 text-sm text-slate-600">
              Adjunta un archivo <strong>.xlsx</strong> con las actividades de la matriz de cargas.
            </p>
            <input
              ref={fileRef}
              type="file"
                ...
                {importResult.error ? (
                  ...
                  <>
                    {importResult.created !== undefined && (
                      <p>Actividades creadas: <strong>{String(importResult.created)}</strong></p>
                    )}
                    {importResult.updated !== undefined && (
                      <p>Actividades actualizadas: <strong>{String(importResult.updated)}</strong></p>
                    )}
